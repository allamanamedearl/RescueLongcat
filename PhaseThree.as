package  {
	...
	public class PhaseThree extends MovieClip{
		...
		//constructor yay! //NO VOID IN THE CONSTRUCTOR WOOHOO!! 
		public function PhaseThree(_stage:Stage) {
		...
		public function Update():void {
			...
			for(i = 0; i < enemyArr.length; i++) {
				if(enemyArr[i].currHP <= 0) {
					if(isBossFight) {
						if(enemyArr[i].eType == "BOSS_FINAL") {
							RemoveAll();
		...
		private function SpawnEnemy():void {
			if(!isBossFight) {
				...
				if(minion_st >= MINION_SPAWN) {
					var enemy:EnemySprite = new EnemySprite(StageVar, 5);
				...
				if(boss_st >= BOSS_SPAWN) {
					var enemy:EnemySprite = new EnemySprite(StageVar, bossType); //4- final boss
					StageVar.addChild(enemy.image);
					enemyArr.push(enemy); 
					boss_st = 0;
					isBossFight = true;
